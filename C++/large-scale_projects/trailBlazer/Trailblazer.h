...
//Function allows us to put the path into the vector
Vector<Loc> generatePath(Map<Loc, Loc>& childParent, Loc start, Loc end);
//Function generates the whole grid and assigns unique integer to each of the Locs
void generateGrid(int numRows, int numCols, TrailblazerPQueue<Edge>& queue, Map<Loc, int>& intIdentifier);
//Function generates edge with first and second being endpoints and puts them in the queue with random priority
