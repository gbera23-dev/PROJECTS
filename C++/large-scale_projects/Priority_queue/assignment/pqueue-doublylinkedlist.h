...
private:
	//this is the struct of doubley - linked node 
	struct node {
		string data;
		node* prev;
		node* next;
		//constructor first takes in the left side's pointer and then right side's 
	...
	node* head; 
	node* tail;
	string popMin(node*& first); //Method pops out the particular node out of the linked list
};
