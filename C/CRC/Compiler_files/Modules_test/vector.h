...
typedef struct {
  //we need logical size and capacity, also the size of element(to know how much memory we need)
  int log_size; 
  int capacity; 
  int elem_size; // 
  void* head; //this is the head of the memory block vector maintains(in heap)
  VectorFreeFunction clean_up; 
} vector;
